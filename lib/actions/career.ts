...
import { OPEN_POSITIONS } from "../../app/career/position";
...
export async function submitCareerApplication(data: CareerFormData): Promise<{ success: boolean; error?: string }> {
    try {
        const { name, email, phone, positionId, portfolioUrl, description, cvBase64, cvFileName } = data;
        ...
        if (!name || !email || !phone || !positionId || !description || !cvBase64) {
            return { success: false, error: 'All primary fields and a valid CV attachment are required.' };
        }
      ...
      `
        ...
        return { success: false, error: 'Failed to process execution pipeline.' };
    }
}
